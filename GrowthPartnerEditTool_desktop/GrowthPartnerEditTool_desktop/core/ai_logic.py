...
- "sharpen" — sharper image"""
    ...
    if content_type == "short_clip":
        ...
        if peak_candidates and wants_best_moments:
            candidates_block = f"""
GEMINI VIDEO ANALYSIS — these are the pre-selected best moments from the video:
{peak_candidates}

⚠️ IMPORTANT: Use ONLY the start/end timestamps from the CANDIDATES above.
Copy them EXACTLY. Do NOT invent new timestamps or use the transcript for timing."""
            transcript_block = ""
        else:
            candidates_block = ""
            transcript_block = f"""VIDEO TRANSCRIPT:
...
{json_schema}"""
    ...
    # ────────────────────────────────
        ...
        num_clips_v = int(re.search(r'(\d+)', user_prompt).group(1)) if re.search(r'(\d+)', user_prompt) else 3
        transcript_short = "\n".join(f"[{s['start']:.0f}s] {s['text']}" for s in segments[:100]) if segments else "(no speech detected)"
        transcript_full = "\n".join(f"[{s['start']:.0f}s] {s['text']}" for s in segments[:300]) if segments else "(no speech detected)"

        if peak_candidates and wants_best_moments:
            candidates_block = f"""
GEMINI VIDEO ANALYSIS — Gemini watched the actual video and pre-selected these moments:
{peak_candidates}

⚠️ IMPORTANT: Pick the best {num_clips_v} from the CANDIDATES above.
Copy start/end timestamps EXACTLY. Do NOT invent new timestamps or use the transcript for timing."""
            transcript_block = f"""VIDEO TRANSCRIPT (background context only — do NOT use for timestamps):
{transcript_short}"""
        else:
            candidates_block = ""
            transcript_block = f"""VIDEO TRANSCRIPT (with timestamps):
{transcript_full}"""
...
GUIDELINES:
- "Find best moments" → if GEMINI CANDIDATES shown above, pick from those ONLY; else select from transcript
...
{json_schema}"""
    ...
    # ────────────────────────────────
        ...
        if peak_candidates and not wants_split:
            # Gemini already watched the video and pre-selected the best moments.
            # Claude's only job here is to RANK and pick num_clips from those candidates.
            selection_criteria = f"""
SELECTION CRITERIA — GEMINI PRE-SELECTED MOMENTS:
Gemini analyzed the actual video and identified the candidates listed above.
Your job is to pick the best {num_clips} from those CANDIDATES ONLY.

- Read every CANDIDATE's REASON carefully
- Pick the {num_clips} that best match the user instruction: "{user_prompt}"
- Copy start/end timestamps EXACTLY from the candidate — do NOT adjust or invent new ones
- Use the LABEL from the candidate as the clip label (you may shorten it)
- Do NOT use timestamps that aren't in the candidates list
- If fewer than {num_clips} candidates exist, use all of them"""
...
- Pick {num_clips} most active blocks and explain why each is a good moment"""
        ...
        clip_length_rule = (
            "Each part should be roughly equal in length, covering the FULL duration of the stream with no gaps."
            if wants_split else
            "Each clip must be 20-50 seconds (end - start)."
        )

        if peak_candidates and not wants_split:
            context_block = f"""{peak_candidates}

STREAM OVERVIEW (for background context only — timestamps above take priority):
{stream_context[:3000] if stream_context else "(no stream context)"}"""
        elif wants_split:
            context_block = stream_context
        else:
            context_block = peak_candidates if peak_candidates else stream_context
